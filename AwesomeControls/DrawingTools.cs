...
namespace AwesomeControls
{
	...
	public sealed class DrawingTools
	{
		...
		{
			g.DrawLine(Pens.HighlightPen, r.Left, r.Top, r.Right - 1, r.Top);
			g.DrawLine(Pens.HighlightPen, r.Left, r.Top, r.Left, r.Bottom - 1);

			g.DrawLine(Pens.ShadowPen, r.Left + 1, r.Bottom - 1, r.Right - 1, r.Bottom - 1);
			g.DrawLine(Pens.ShadowPen, r.Right - 1, r.Top + 1, r.Right - 1, r.Bottom - 1);
		}
        ...
        public static void DrawArrow(Graphics graphics, Color color, Direction direction, int x, int y, int size)
        {
            ...
            switch (direction)
            {
                ...
                case Direction.Down:
                    ...
                    {
                        // Intermediate lines
                        graphics.DrawLine(p, xpos, ypos, xpos + (size + o), ypos);
                        xpos++;
                        ypos++;
                        o -= 2;
                    }
                    // Last line
                    graphics.DrawLine(p, x + (size - 1), y, x + (size - 1), y + (size - 1));
                    break;
                case Direction.Left:
